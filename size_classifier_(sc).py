...
Original file is located at
    https://colab.research.google.com/drive/1QjDKZbjRP7e6VE6aPOW7lZLHduhLb6bo
"""
...
#BUF
with open('/content/drive/My Drive/CPS 580 Project File/Species Specific Size Classifier/SC_X.pkl', 'rb') as X1:
  SC_X = pickle.load(X1, encoding = 'latin1')

with open('/content/drive/My Drive/CPS 580 Project File/Species Specific Size Classifier/SC_Y.pkl', 'rb') as X1:
  SC_Y = pickle.load(X1, encoding = 'latin1')
...
for i in range(k):
  ...
  # Train the model with the training data, set epochs and batch size
  history = cnn_model.fit(datagen.flow(train_data, train_targets, batch_size = batch_size), steps_per_epoch = len(train_X)/batch_size,
                          epochs = num_epochs, validation_data = (val_data, val_targets), verbose = 0)
  cnn_model.evaluate(test_X, test_Y)
...
plt.title("Training and Validation Accuracy (HypTest1)")
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 40, batch_size = 30, validation_data=(val_X, val_Y))
...
cnn_model.add(Flatten())
cnn_model.add(Dense(64, activation='selu', kernel_regularizer=regularizers.l2(0.01)))
cnn_model.add(Dropout(0.5))
cnn_model.add(Dense(64, activation='selu', kernel_regularizer = regularizers.l2(0.01)))
cnn_model.add(Dropout(0.5))
cnn_model.add(Dense(3, activation = 'softmax'))
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 60, batch_size = 30, validation_data=(val_X, val_Y))
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 60, batch_size = 20, validation_data=(val_X, val_Y))
...
"""VGG16 (Best Model) with K-fold validation"""
...
def build_model():
  ...
  cnn_model.add(Dense(64, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
  cnn_model.add(Dropout(0.5))
  cnn_model.add(Dense(32, kernel_regularizer = regularizers.l2(0.01), activation='relu'))
  cnn_model.add(Dropout(0.5))
  cnn_model.add(Dense(64, activation='relu', kernel_regularizer = regularizers.l2(0.01)))
  cnn_model.add(Dense(3, activation = 'softmax'))
