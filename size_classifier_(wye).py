...
Original file is located at
    https://colab.research.google.com/drive/1gHzcVPMJKt64YNhm5Mvjxc_1cwFGHx3U
"""
...
#WYE
with open('/content/drive/My Drive/CPS 580 Project File/Species Specific Size Classifier/WYE_X.pkl', 'rb') as X1:
  WYE_X = pickle.load(X1, encoding = 'latin1')

with open('/content/drive/My Drive/CPS 580 Project File/Species Specific Size Classifier/WYE_Y.pkl', 'rb') as X1:
  WYE_Y = pickle.load(X1, encoding = 'latin1')
...
history = cnn_model.fit(train_X, train_Y, epochs = 100, batch_size = 20, validation_data= (val_X, val_Y))
...
train_data = WYE_X[:857, ]
train_targets = WYE_Y[:857, ]
test_X = WYE_X[-214:,]
test_Y = WYE_Y[-214:,]
...
for i in range(k):
  ...
  #history = cnn_model.fit(datagen.flow(train_data, train_targets, batch_size = batch_size), steps_per_epoch = len(train_X)/batch_size, epochs = num_epochs, validation_data = (val_data, val_targets), verbose = 0)
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 75, batch_size = 25, validation_data=(val_X, val_Y))
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 60, batch_size = 50, validation_data=(val_X, val_Y))
...
# Train the model with the training data, set epochs and batch size
history = cnn_model.fit(train_X, train_Y, epochs = 70, batch_size = 50, validation_data=(val_X, val_Y))
...
for i in range(k):
  ...
  # Train the model with the training data, set epochs and batch size
  history = cnn_model.fit(partial_train_data, partial_train_targets, epochs = num_epochs, batch_size = 25, validation_data=(val_data, val_targets), verbose = 0)
